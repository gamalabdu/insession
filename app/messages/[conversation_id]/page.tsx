...
const ConversationPage = (props : ConversationPageProps) => {
  ...
  const { conversation } = useGetConversationByConversationId(params.conversation_id)

  const mainUser = conversation?.participant_ids[0] === user?.id ? conversation?.participant_ids[0] : conversation?.participant_ids[1]

  const secondUser = conversation?.participant_ids[0] != user?.id ? conversation?.participant_ids[0] : conversation?.participant_ids[1]

  const mainUserPhoto = useGetUserProfileInfo(mainUser).userProfileInfo?.avatar_url

  const secondUserPhoto = useGetUserProfileInfo(secondUser).userProfileInfo?.avatar_url
  ...
  return (
    ...
    <div className="flex flex-col bg-neutral-900 rounded-lg h-full w-full">
      ...
      <div className="w-full bottom-10 h-[50px] flex flex-row align-middle p-2 gap-2">
          ...
          <label htmlFor="file-input" className="cursor-pointer flex flex-col align-middle justify-center">
            <FiFilePlus size={22} className="text-neutral-500 mt-1 hover:text-neutral-200" />
          </label>
          ...
          <Input type='text' value={messageContent} placeholder='type your message here...' onChange={(e) => setMessageContent(e.target.value)} />

          <div className="cursor-pointer flex flex-col align-middle justify-center">
              <GrSend onClick={ () => sendMessage() } size={20} className="text-neutral-500 mt-1 hover:text-neutral-200" />
          </div>
